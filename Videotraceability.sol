...
contract Vid_Traceability {
    ...
    constructor() {
        ...
        thisVideo.IPFS_Hash = "asfasfasfs";
        thisVideo.info = "A comic Video";
        thisVideo.SC_address = msg.sender;
        thisVideo.metadata = "Data:18/9/2015, Time 12:05";
    ...
    function AttestSC(
        /*string memory infor*/
        bytes32 hash, /*string memory meta*/
        address SCaddress
    ) public NotOwner {
        require(
            Granted_Permissions[hash].state == artistState.GrantedPermission
        ); //requires state to be granted
        emit AttestationRequest("Address of child: ", SCaddress);
        Granted_Permissions[hash].state = artistState.SentAttestationRequest;
    }

    function GrantAttestation(
        bool result,
        string memory infor,
        bytes32 hash,
        string memory meta,
        ...
        require(
            Granted_Permissions[hash].state ==
                artistState.SentAttestationRequest
        );
        if (result) {
